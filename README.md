...
- [ ] 제목이 빈 문자열이면 예외
- [ ] 유저 이름이 공백이면 예외
- [ ] 카드에 유저를 제외한 필드가 모두 비어있으면(null, blank) 예외

### UI 테스트

[ ] 모든 필드가 있는 카드 - 제목 설명 태그 유저 모두 노출
[ ] 제목 필드가 없는 카드 - "제목 없음" 표시
[ ] 유저가 null인 카드 - "알 수 없는 유저" 노출
[ ] 내용이 없는 카드 - 내용 영역이 노출되지 않음
[ ] 태그가 없는 카드 - 태그 영역이 노출되지 않음
